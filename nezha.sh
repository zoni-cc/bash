...
update_script() {
    ...
    curl -sL https://raw.githubusercontent.com/zoni-cc/bash/main/nezha.sh -o /tmp/nezha.sh
    new_version=$(cat /tmp/nezha.sh | grep "NZ_VERSION" | head -n 1 | awk -F "=" '{print $2}' | sed 's/\"//g;s/,//g;s/ //g')
    if [ ! -n "$new_version" ]; then
        echo -e "脚本获取失败，请检查本机能否链接 https://raw.githubusercontent.com/zoni-cc/bash/main/nezha.sh"
        return 1
    fi
...
modify_agent_config() {
    ...
    sed -i "s/nz_client_secret/${nz_client_secret} --skip-conn/" ${NZ_AGENT_SERVICE}
